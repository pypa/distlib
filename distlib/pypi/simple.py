...
from .errors import (DownloadError, UnableToDownload, CantParseArchiveName,
                     ReleaseNotFound, ProjectNotFound, PyPIError)
...
DEFAULT_HOSTS = ("*",)
SOCKET_TIMEOUT = 15
USER_AGENT = "Python-urllib/%s.%s distlib/%s" % (
    sys.version_info[0], sys.version_info[1], distlib_version)
...
class Crawler(BaseClient):
    ...
    def _is_browsable(self, url):
        ...
        if self.index_url in url or urlparse(url)[0] == "file":
            return True
        elif self.follow_externals:
            if self._allowed_hosts(urlparse(url)[1]):  # 1 is netloc
                return True
            else:
                return False
        return False
    ...
    def _get_full_url(self, url, base_url):
        return urljoin(base_url, self._htmldecode(url))
    ...
    def _open_url(self, url):
        ...
        if auth:
            auth = "Basic " + \
                unquote(auth).encode('base64').strip()
