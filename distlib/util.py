...
def _getobj(proxy):
    try:
        result = object.__getattribute__(proxy, '_obj')
    except AttributeError:
        func = object.__getattribute__(proxy, '_func')
        result = func()
        object.__setattr__(proxy, '_obj', result)
    return result

class LazyProxy(object):
    #
    # Adapted from http://code.activestate.com/recipes/496741-object-proxying/
    # by Tomer Filiba
    #
    __slots__ = ['_obj', '_func', '__weakref__']
    ...
    def __getattribute__(self, name):
        return getattr(_getobj(self), name)
    def __delattr__(self, name):
        delattr(_getobj(self), name)
    def __setattr__(self, name, value):
        setattr(_getobj(self), name, value)

    def __nonzero__(self):
        return bool(_getobj(self))
    def __str__(self):
        return str(_getobj(self))
    def __repr__(self):
        return repr(_getobj(self))
    ...
    def _create_class_proxy(cls, theclass):
        ...
        def make_method(name):
            def method(self, *args, **kw):
                return getattr(_getobj(self), name)(*args, **kw)
            return method
